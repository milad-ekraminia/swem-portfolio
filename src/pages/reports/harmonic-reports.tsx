...
import {
  harmonicReportFields,
} from '@/enum-data/reports/harmonic';
import { dateFormatter } from '@/helpers/format-data';
import { getTranslatedValue } from '@/helpers/get-translated-value';
import { getHarmonikReportApi } from '@/services/reports/get-reports-apis';
import { BreadcrumbItem } from '@/types/components/ui/bread-crumb';
import { reportsAddProfileInitialValuesTypes } from '@/types/pages/reports/reports';
import { reportsAddProfileHarmonicInitialValues } from '@/validations/reports/reports-validation';
...
export default function HarmonicReports() {
  ...
  const {
    ...
    defaultValues:
      reportsAddProfileHarmonicInitialValues as reportsAddProfileInitialValuesTypes,
    context: {
  ...
  const {
    ...
    queryKey: ['Harmonic Report Table', currentPage, sortData, pageSize],
    queryFn: () =>
      getHarmonikReportApi({
        dataParams: {
  ...
  const {
    ...
    queryKey: ['Harmonic Report Chart', chartCurrentPage],
    queryFn: () =>
      ...
      <div className="page-wrapper__body">
        <div className="reports">
          <div className="reports-form">
            <ReportsFormWrapper
              title={getTranslatedValue('em_report_elec_harmonic')}
              setValue={setValue}
              handleSubmit={handleSubmit}
              register={register}
              getValues={getValues}
              control={control}
              optionsList={harmonicReportFields}
              hasPeriodType={false}
              filterName="device-elec-harmonics"
              errors={errors}
            ...
            <div className="reports-response">
              <ReportResults
                ...
                excelParams={{
                  excelFormData,
                  watchInfo,
                  excelFileName: 'em_report_elec_harmonic',
                  excelUrl: `app/export-to-excel/export-to-excel-device-elec-harmonics-list?api-version=${import.meta.env.VITE_API_VERSION}`,
                }}
                chartType="area"
              />
            </div>
