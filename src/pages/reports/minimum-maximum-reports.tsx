...
import { minMaxReportFields } from '@/enum-data/reports/minimum-maximum';
import { dateFormatter } from '@/helpers/format-data';
import { getTranslatedValue } from '@/helpers/get-translated-value';
import { getMinimumMaximumReportApi } from '@/services/reports/get-reports-apis';
import { BreadcrumbItem } from '@/types/components/ui/bread-crumb';
import { reportsAddProfileInitialValuesTypes } from '@/types/pages/reports/reports';
import { reportsAddProfileMinimumMaximumInitialValues } from '@/validations/reports/reports-validation';
...
export default function MinimumMaximumReports() {
  ...
  const {
    ...
    defaultValues:
      reportsAddProfileMinimumMaximumInitialValues as reportsAddProfileInitialValuesTypes,
    context: {
  ...
  const {
    ...
    queryKey: ['Minimum Maximum Report Table', currentPage, sortData, pageSize],
    queryFn: () =>
      getMinimumMaximumReportApi({
        dataParams: {
  ...
  const {
    ...
    queryKey: ['Minimum Maximum Report Chart', chartCurrentPage],
    queryFn: () =>
  ...
  const staticTableHeaders: any[] = useMemo(
    () => [
      ...
      },
      {
        ...
        sort: 'ReadDateTime',
        cell: ({ row }: any) => {
          return dateFormatter(row?.original?.readDateTime);
        },
      },
    ],
    [],
            ...
            <div className="reports-response">
              <ReportResults
                ...
                excelParams={{
                  excelFormData,
                  watchInfo,
                  excelFileName: 'MinMaxValueReport',
                  excelUrl: `app/export-to-excel/export-to-excel-device-elec-min-max-values-list?api-version=${import.meta.env.VITE_API_VERSION}`,
                }}
                chartType="area"
              />
            </div>
