...
const MemoChartViewActivePower = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      ...
      treeData?.tree_id,
      'em_instant_active_power',
    ],
    queryFn: () =>
      fetchInverterInstantValueLineChart({
        tree_id: treeData?.tree_id,
        selectedFilters: inverterInstantChartFilterValueHandler(
          'em_instant_active_power',
        ),
      }),
    retry: false,
  ...
  return (
    <div className="device-inverter-charts-container-grid__item">
      <LineChart
        series={series}
        dates={uniqueDates}
        title={getTranslatedValue(`ActivePowerGraph`)}
        isLoading={isLoading}
        chartId="active-power-chart"
...
const ChartViewActivePower = memo(MemoChartViewActivePower);

export default ChartViewActivePower;
