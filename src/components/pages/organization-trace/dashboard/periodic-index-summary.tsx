...
import { fetchDashboardPeriodicIndexSummary } from '@/services/organization-trace/dashboard-api';
import { useQuery } from '@tanstack/react-query';
import { memo, useState } from 'react';
import { CardDayDetailsModal } from '../cards/card-day-detail-modal';
import { CardMonthDetailsModal } from '../cards/card-month-detail-modal';
import { CardYearDetailsModal } from '../cards/card-year-detail-modal';
...
const PeriodicIndexSummary = ({
  ...
  const { data, isLoading, isPending } = useQuery({
    queryKey: ['Organization Cards Details', orgId],
    queryFn: () => fetchDashboardPeriodicIndexSummary({ orgId }),
    retry: false,
    enabled: !!orgId,
    ...
    IndCurrentMonthActive1ExpCons: {
      titleValue:
        (data?.indCurrentMonthActive1ExpCons ?? 0) > 0
          ...
          : 0,
      descValue: (data?.monthlyProductionForecast ?? 0) > 0 ? (data?.monthlyProductionForecast ?? 0) / 1000 : 0,
    },
    IndCurrentYearActive1ExpCons: {
      titleValue:
        (data?.indCurrentYearActive1ExpCons ?? 0) > 0
          ...
          : 0,
      descValue: (data?.yearlyProductionForecast ?? 0) > 0
            ...
            <>
              {isLoading || isPending ? (
                ...
                <Card
                  ...
                  chartStatus={true}
                  type={'progressbar'}
                  showDetailHandler={() => showDetailHandler(card.title)}
                />
              )}
            </>
          );
      ...
      {showMonthModal && (
        ...
        >
          <CardMonthDetailsModal
            ...
            monthlyProductionForecast={(data?.monthlyProductionForecast ?? 0) > 0 ? (data?.monthlyProductionForecast ?? 0) / 1000 : 0}
      ...
      {showYearModal && (
        ...
        >
          <CardYearDetailsModal
            ...
            yearlyProductionForecast={(data?.yearlyProductionForecast) > 0 ? (data?.yearlyProductionForecast) / 1000 : 0}
