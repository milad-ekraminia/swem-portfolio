...
import {
  demandDataReportFields,
} from '@/enum-data/reports/demand';
...
import { getDemandReportApi } from '@/services/reports/get-reports-apis';
import { reportsAddProfileInitialValuesTypes } from '@/types/pages/reports/reports';
import { reportsAddProfileDemandInitialValues } from '@/validations/reports/reports-validation';
...
export default function DemandDataReportsTable() {
  ...
  const {
    ...
    defaultValues:
      reportsAddProfileDemandInitialValues as reportsAddProfileInitialValuesTypes,
    context: {
  ...
  const {
    ...
    queryKey: ['Demand Data Report Table', currentPage, sortData, pageSize],
    queryFn: () =>
      getDemandReportApi({
        dataParams: {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      downloadExcelFile({
        response: data,
        fileName: getTranslatedValue('mnuEMReportElecDemand'),
      });
    },
  ...
  const handleDownloadExcel = async () => {
    mutation.mutate({
      url: `app/export-to-excel/export-to-excel-device-elec-demand-instants-list?api-version=${import.meta.env.VITE_API_VERSION}`,
      formData: excelFormData,
    });
  };

  const memoizedBaseColumns = useMemo(
    () => getTagReportsColumns(),
    [tableData],
  );
  ...
  return (
    <div className="reports">
      <div className="reports-form">
        <ReportsFormWrapper
          title={getTranslatedValue('mnuEMReportElecDemand')}
          setValue={setValue}
          ...
          control={control}
          hasPeriodType={false}
          optionsList={demandDataReportFields}
          filterName="device-elec-demand-instants"
          errors={errors}
