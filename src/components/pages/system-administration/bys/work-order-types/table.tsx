...
import { workOrderTypes as basicColumns } from '@/enum-data/bys/work-order-types';
import { getPermission } from '@/helpers/get-permission-helper';
import { getTranslatedValue } from '@/helpers/get-translated-value';
import { useDownloadFile } from '@/hooks/use-download-file';
import { fetchWorkOrderTypesList } from '@/services/system-administration/bys/work-order-types-api';
import { ISort } from '@/types/components/ui/table';
import { useQuery } from '@tanstack/react-query';
import { useMemo, useState } from 'react';
import UpdateWorkOrderTypeModal from './edit-modal';

export const WorkOrderTypesTable = ({ setNewItem }: { setNewItem: any }) => {
  ...
  const handleDownload = () => {
    downloadHandler({
      excelUrl: `app/work-order-types/as-excel-file`,
      fileName: getTranslatedValue('WorkOrderTypes'),
      searchInputValue: searchInputValue,
      getTokenUrl: `app/work-order-types/download-token?api-version=${import.meta.env.VITE_API_VERSION
        }`,
    });
  };
  ...
  const header = useMemo(
    () => (
      <GlobalTableHeader
        ...
        label="WorkOrderTypes"
        tableColumns={memoizedBaseColumns}
        newButtonLabel="NewWorkOrderType"
  ...
  return (
    <>
      <Table
        ...
        pageSize={pageSize}
      />
      {getPermission('WebNet.WorkOrderTypes.Edit') && showEditModal && (
        <Modal
