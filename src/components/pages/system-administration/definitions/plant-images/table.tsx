...
import { getColumns } from '@/enum-data/system-administration/plant-images';
import { getPermission } from '@/helpers/get-permission-helper';
import { getTranslatedValue } from '@/helpers/get-translated-value';
import { useDownloadFile } from '@/hooks/use-download-file';
import {
  getPartialPlantsList,
  getPlantImages,
...
import PlantImageForm from './form';

export default function PlantImagesTable() {
  ...
  const [showEditModal, setShowEditModal] = useState<null | PlantImage>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [pageSize, setPageSize] = useState(10);

  const { data, isLoading } = useQuery({
    queryKey: ['plantImages', searchTerm, sortData, pageSize, currentPage],
    queryFn: () =>
      getPlantImages({
  ...
  const handleDownload = () => {
    downloadHandler({
      excelUrl: `app/plant-images/as-excel-file`,
      fileName: getTranslatedValue('PlantImages'),
      searchInputValue: searchTerm,
      getTokenUrl: `app/plant-images/download-token?api-version=${import.meta.env.VITE_API_VERSION}`,
    });
  };
  ...
  const header = useMemo(
    () => (
      <GlobalTableHeader
        label={getTranslatedValue('PlantImages')}
        tableColumns={memoizedBaseColumns}
        setSelectedColumnKeys={setSelectedColumnKeys}
        selectedColumnKeys={selectedColumnKeys}
        columnOrder={columnOrder}
        newButtonLabel={getTranslatedValue('NewPlantImage')}
        setColumnOrder={setColumnOrder}
        searchInputHandler={setSearchTerm}
        isExcelDownloading={isExcelDownloading}
        handleDownload={handleDownload}
        setNewItem={getPermission('WebNet.PlantImages.Create') ? handleCreatingModal : undefined}
      />
    ),
  ...
  return (
    <>
      <Table
        ...
        hasPagination={true}
      />
      {getPermission('WebNet.PlantImages.Create') && creatingNewModel && (
        <Modal
          modalSize="sm"
      ...
      {!!showDeleteModal && (
        ...
        >
          <SureDeleteModal
            ...
            deleteItemUrl={`app/plant-images/${showDeleteModal}?api-version=${import.meta.env.VITE_API_VERSION}`}
          />
        </Modal>
      )}
    </>
