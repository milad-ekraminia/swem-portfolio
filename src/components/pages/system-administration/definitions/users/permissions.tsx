...
const MemoPermissions = ({ userInfo, setShowModal }: Props) => {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    queryFn: () => getUserPermission(userInfo.id),
    enabled: !!userInfo.id,
    retry: false,
  });
    ...
    return permissions.map((permission) => {
      const isCurrentUser =
        permission.grantedProviders
          ?.filter((elem) => elem.providerName === 'U')
          ?.find((elem) => elem.providerName === 'U')?.providerKey ===
        userInfo.id;
      const isDisabled =
        permission?.isGranted && permission.grantedProviders?.length !== 0; // اگه کاربر یا نقش مجوزی رو نداشته باشه چک باکس مربوطه قابل ویرایش نمایش داده میشه این رو از isGranted بدست میاریم
      return (
        <div
          key={permission.name}
  ...
  const mutationUpdateUserPermission = useMutation({
  ...
  const submitHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    mutationUpdateUserPermission.mutate({
      userId: userInfo.id,
      formData: list,
    });
  };
  ...
  return (
    <form className="global-modal" onSubmit={submitHandler}>
      ...
      <SubmitOrCancelButtons
        ...
        isPending={mutationUpdateUserPermission.isPending}
