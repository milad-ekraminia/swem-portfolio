...
import { dashboardMapTableColumns as baseColumns } from '@/enum-data/system-administration/dashboard-map-data';
...
export const MemoDashboardMapTable = ({
  ng,
  re,
  activeTab,
  setNewItem,
  ...
  const { data: oneOrganizationData, isSuccess: oneOrganizationIsSuccess } =
    useQuery({
  ...
  const {
    data: searchedOrganizationsResultData,
    isLoading: searchedOrganizationsResultLoading,
  } = useQuery({
    queryKey: [
      'All Organizations',
      searchInputValue,
  ...
  return (
    <>
      <Table
        ...
        isLoading={
          searchedOrganizationsResultLoading || isLoading
          // ||
...
const DashboardMapTable = memo(MemoDashboardMapTable);

export default DashboardMapTable;
