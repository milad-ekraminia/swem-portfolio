...
>({
  ...
  const emitOptionDelete = (opt: InternalOption) => {
  ...
  return (
    <div
      className={`select-input ${mainClass ?? ''} ${suffix ? 'has-suffix' : ''}`}
    >
      {label && (
        ...
        {isOpen &&
          createPortal(
            <ul
              ...
              className={`select-input__dropdown ${openDirection === 'up' ? 'dropdown-up' : 'dropdown-down'} ${isFull ? 'dropdown-full' : ''}`}
            >
              {hasResults ? (
                isFull ? (
