...
const Table = <T extends { children?: T[] }>({
  ...
  const table = useReactTable<T>({
    ...
    onExpandedChange: (updater) => {
      setExpanded((old: any) => {
        ...
        if (singleExpand) {
          const clickedId = Object.keys(newExpanded).find(
            (key) => !old[key] && newExpanded[key],
          );

          if (clickedId) {
            // New row clicked → open only it

            return { [clickedId]: true };
          }

          const closedId = Object.keys(old).find(
            (key) => old[key] && !newExpanded[key],
          );

          if (closedId) {
            // Same row clicked → close it
  ...
  return (
    ...
    >
        ...
        {isLoading ? (
          <div
            ...
            style={{
              ...
              left: '50%',
              bottom: 0,
              transform: isRTL ? 'translate(0,-50%) ' : 'translate(-50%,-50%) ',
              zIndex: '100',
            }}
          >
            {renderLoading?.()}
      ...
      {totalCount > 10 && hasPagination && !isLoading ? (
        <Pagination
        ...
        // <div className="pagination-container-placeholder"></div>
        <></>
      )}
    </div>
  );
